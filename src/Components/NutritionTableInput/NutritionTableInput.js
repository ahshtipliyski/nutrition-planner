...
export const NutritionTableInput = ({firstmeal, uid}) => {
  ...
  const onUpdate = () => {
    ...
    db.collection('users').doc(uid).collection('firstmeals').doc(firstmeal.id).set({...firstmeal, name, calories, protein, carbohydrates, fat, sugar})
  }

  const onDelete = () => {
    const db = firebase.firestore()
    db.collection('users').doc(uid).collection('firstmeals').doc(firstmeal.id).delete()
  }
